...
function estimateVelocityByCharge(table, charge, milsPerCircle = 6400) {
  ...
  for (let i = 0; i < Math.min(elev.length, range.length); i += 1) {
    const r = range[i];
    const theta = (elev[i] * 2 * Math.PI) / milsPerCircle;
...
export async function getWeapon(weaponId) {
  ...
  if ((!normalized.charges || !normalized.charges.length) && payload.primaryTable) {
    ...
    const milsPerCircle = normalized.milSystem?.milsPerCircle ?? 6400;
    normalized.charges = charges.map((chargeId) => ({
      id: String(chargeId),
      muzzleVel: estimateVelocityByCharge(payload.primaryTable, String(chargeId), milsPerCircle),
    }));
